...
# 1) 空镜缓存 mm_cache（MiniMax 实拍）/ collage_cache（纸拼贴）/ hf_cache（历史遗留）：
#    删 N 天未访问的 mp4（同句命中缓存会更新 mtime，热数据不会误删）
docker compose exec -T backend sh -c "
  find /app/remotion/public/mm_cache /app/remotion/public/collage_cache /app/remotion/public/hf_cache -type f -name '*.mp4' -mtime +${DAYS} -delete 2>/dev/null || true
"
